...
{
    DlgRestextrakt dlg(bh->sud()->getSWKochbeginn(), 0.0, -1.0, this);
...
{
    DlgRestextrakt dlg(bh->sud()->getSWKochende(), 0.0, -1.0, this);
    if (dlg.exec() == QDialog::Accepted)
...
{
    DlgRestextrakt dlg(bh->sud()->getSWAnstellen(), 0.0, -1.0, this);
    if (dlg.exec() == QDialog::Accepted)
